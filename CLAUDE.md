...
- **`src/hardware_port.rs`** — one port's identity and live state. `ip_address` is an `Option<String>` (`None` = no address, rendered as an empty cell via `display_ip`); `get_ipaddr()` runs `ipconfig getifaddr <device>`; `get_speed()` parses the `media` lines of `ifconfig <device>` via the pure `map_speed_string()`. `service_order` is hidden from the table via `#[tabled(skip)]`.
- **`src/hardware_port_list.rs`** — discovery and ordering. Data flow:
  ...
  3. **`.active_only()`** — keeps only ports with an IP address; `main` skips this step when `--all-ports` is passed, and prints a stderr hint when the filtered list comes back empty.
